...
var Model = Model.extend({
    ...
    create : function() {
       // this._common = new Common();
         ...
         * isNull : null 허용
         */
        this.addField('PlayerID',Type.Int,false,['notEmpty'],true); // 선수코드
        this.addField('DATA', Type.Json,false); // 선수데이터
        this.addField('PlayDate',Type.Timestamp,false); // 경기날짜
        this.addField('WriteDate', Type.Timestamp,false); // 입력시간
        this.addField('UserName', Type.Str,false);
    ...
    getPlayerList: function(PLAYERID, callback) {
        this.query({PlayerID: PLAYERID}, function (err, models) {
            if (models && typeof models.length !== 'undefined' && models.length > 0) {
                callback(models);
            } else {
                callback(null);
